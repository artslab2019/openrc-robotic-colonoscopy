...
def visualize_dataset(
    ...
    for batch in tqdm.tqdm(dataloader, total=len(dataloader)):
        ...
        for i in range(len(batch["index"])):
            ...
            # If needed, use this block to visualize more data
            key_ndi = "observation.ndi_cartesian_relative"
            # Optional: Map indices to names for clearer graphs (from your info.json)
            ndi_names = ["dx", "dy", "dz", "droll", "dpitch", "dyaw"]

            if key_ndi in batch:
                for dim_idx, val in enumerate(batch[key_ndi][i]):
                    # If we have a name, use it; otherwise use the index number
                    name = ndi_names[dim_idx] if dim_idx < len(ndi_names) else str(dim_idx)
                    
                    # Log to Rerun
                    # We group them under "ndi_rel" so they appear in their own folder
                    rr.log(f"ndi_rel/{name}", rr.Scalar(val.item()))
            # ------------------------------------------------
